...
describe("GitHubModelsModel", () => {
  ...
  it("stores generation settings (temperature, maxTokens, topP, maxSteps)", () => {
    const model = new GitHubModelsModel({
      model: "openai/gpt-5-mini",
      settings: { temperature: 1, maxTokens: 4096, topP: 1, maxSteps: 15 },
    });
  ...
  it("stores tools when provided", () => {
    const mockTool = { description: "mock", parameters: {}, execute: vi.fn() };
    const model = new GitHubModelsModel({
      model: "openai/gpt-5-mini",
  ...
  it("has undefined tools when not provided", () => {
    const model = new GitHubModelsModel({ model: "openai/gpt-4o" });
